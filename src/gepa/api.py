...
def optimize(
    ...
    - batch_sampler: Strategy for selecting training examples. Can be a [BatchSampler](src/gepa/strategies/batch_sampler.py) instance or a string for a predefined strategy from ['epoch_shuffled', 'worst_first']. Defaults to 'epoch_shuffled', which creates an [EpochShuffledBatchSampler](src/gepa/strategies/batch_sampler.py). 'worst_first' creates a [WorstFirstBatchSampler](src/gepa/strategies/batch_sampler.py), which biases each reflection minibatch toward the examples the parent candidate performs worst on (falling back to the pool's best-achieved scores where the parent is unmeasured; set relative_to='pool' to target only globally unsolved examples) — intended for the combined train+val pool workflow (valset=None) where per-example validation scores cover the training pool.
